...
struct run_args creater_run_args(int l, int a, int b, int n, int N,
                                 float pl, float pr, float pu, float pd, int rank, int size) 
{
    struct run_args arg;
    arg.l = l;
    arg.a = a;
    ...
    while (1) 
    {
        ...
        while (i < areas_size) 
        {
            ...
            for (int t = 0; t < 190; t++) 
            {
                ...
                int direction = max(rand_r((unsigned int*) &areas_i->rand) * pl, rand_r((unsigned int*) &areas_i->rand) * pr,
                              rand_r((unsigned int*) &areas_i->rand) * pu, rand_r((unsigned int*) &areas_i->rand) * pd);
                if (direction == 0) 
                    areas_i->points.x -= 1;
                 else if (direction == 1) 
                    areas_i->points.x += 1;
...
int main(int argc, char** argv) 
{
    ...
    int n = atoi(argv[4]);
    int N = atoi(argv[5]);
    float pl = atof(argv[6]);
    float pr = atof(argv[7]);
    float pu = atof(argv[8]);
    float pd = atof(argv[9]);

    struct run_args arg = creater_run_args(l, a, b, n, N, pl, pr, pu, pd, rank, size);
